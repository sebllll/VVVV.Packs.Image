...
namespace VVVV.CV.Nodes.Tracking
{
    ...
    public TrackingPedestrianInstance()
        ...
        {
            //this.Logger.Log(LogType.Debug, "create new HOG");
            if (GpuInvoke.HasCuda && AllowGpu)
            {
                ...
                //gpuhog.SetSVMDetector(GpuHOGDescriptor.GetPeopleDetector64x128());
        ...
        private Rectangle[] DetectPedestrian(Image<Bgr, byte> bgrImage)
        {
            ...
            //bgrImage._EqualizeHist(); // what does it do?
    ...
    public class PedestrianTrackingNode : IDestinationNode<TrackingPedestrianInstance>
    {
        ...
        [Input("Block Stride", DefaultValues = new double[] { 8, 8 })]
        private IDiffSpread<Vector2D> FBlockStride;

        [Input("Block Stride Multiplicator", DefaultValue = 1)]
        ...
        private void CheckParams(int instanceCount)
        {
            for (var i = 0; i < instanceCount; i++)
            {
                try
                {
                    //if (FHitThreshold.IsChanged) FProcessor[i].hitThreshold = FHitThreshold[i];
                    //if (FWinStride.IsChanged) FProcessor[i].winStride = new Size((int)FWinStride[i].x, (int)FWinStride[i].y);
                    //if (FPadding.IsChanged) FProcessor[i].padding = new Size((int)FPadding[i].x, (int)FPadding[i].y);
                    //if (FScale.IsChanged) FProcessor[i].scale = FScale[i];
                    //if (FFinalThreshold.IsChanged) FProcessor[i].finalThreshold = FFinalThreshold[i];
                    //if (FUseMeanShiftGrouping.IsChanged) FProcessor[i].useMeanShiftGrouping = FUseMeanShiftGrouping[i];
                    //if (FAllowGpuIn.IsChanged) FProcessor[i].AllowGpu = FAllowGpuIn[i];

                    //if (FProcessor[i].Logger == null)
                    //{
                    //    FProcessor[i].Logger = FLogger;
                    //}
